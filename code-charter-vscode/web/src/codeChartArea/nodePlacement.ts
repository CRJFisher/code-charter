...
function generateRelativePlacementConstraints(
    ...
    function traverse(node: DefinitionNode) {
      ...
      const childSymbols = node.children
        .map((childRef) => childRef.symbol)
        .filter((symbol) => !nodesToSkip.includes(symbol));
      ...
      for (const childSymbol of childSymbols) {
        const childNode = callGraph.definitionNodes[childSymbol];
        if (childNode) {
          traverse(childNode);
        }
      }
  ...
  export {
    generateRelativePlacementConstraints,
    generateElements,
  }