...
**Remaining scope (AC #8–#9):** the built taxonomy covers the site-recorded failure shapes plus the control; the evidence-less class (`interface_method/`, seeds-only umbrella with no corroborable bridge) and the re-export class (`barrel_reexport/`, signal pinned on authoring) are planned but not yet built. The Tier 2 harness's expectation table requires ≥ 1 bridge for every stitch fixture, so the evidence-less fixture also needs a seeds-only expectation kind in `src/bin/stitch_eval.ts`.
