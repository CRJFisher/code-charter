...
- **#3** — **keep + one revision (the payload enrichment) + two follow-ups** (task-27.1.6.3 richer recovery, task-27.1.6.4 standalone-doc flows).
...
- **#6** — standalone docs take option (a): ignored and logged, never gating a turn; option (b) (doc nodes + doc↔code linkage) is deferred to task-27.1.6.4.
...
- Two hook-UX rough edges (AC#5/#6) surfaced and are addressed here: the hook blocked for non-flow files (a doc/config-only turn → a no-op full-repo reconcile), and a standalone new `.md` (this very task doc, e.g.) maps to no flow because v1 only forms skill-dir and code-entrypoint flows — so the hook fired with nothing to do. The flow-relevance pre-filter drops both, and standalone-doc flows are deferred to task-27.1.6.4.
