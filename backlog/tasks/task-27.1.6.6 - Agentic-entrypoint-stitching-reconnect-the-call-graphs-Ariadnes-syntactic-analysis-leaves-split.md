...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 **Executor seams purged.** `EntrypointStitchExecutor`, `null_stitch_executor`, `stitch_entrypoints?`, `DescribeBatchExecutor`, `null_describe_executor`, `describe?`, the `reconcile.ts` executor fallback, and their re-exports are deleted. No `*_executor` injected-function seam remains on `ReconcileDeps`; a repo grep for the removed identifiers returns only this task's history.
- [x] #2 **Candidate/merge/agenda layer purged.** `build_candidate_stitches`, `StitchCandidate`, `StitchBatch`, `MAX_STITCH_CANDIDATES`, `build_stitch_groups`, and `detect_and_stitch_code_umbrellas` are deleted; `packages/core/src/agentic/stitch.ts` deletes (or shrinks to only still-consumed exports). No candidate-pairing or union-find runs in the reconcile path.
- [x] #3 **`drift-reconcile` exposes exactly three agentic modes.** `--list-entrypoints` emits `{ entrypoints: [{ symbol_path, name, file, line, is_orphan, unresolved_sites: [{ file, line, source_line }] }] }` for the changed neighbourhood and runs the full deterministic reconcile (resync/retire/skill-dir plus one-singleton-flow-per-new-entrypoint hydration — hydrate-then-merge, so an agent crash mid-phase leaves today's correct fragmented state and agent-less hosts keep getting flows). `--apply-stitch <json>` consumes `{ umbrellas: [{ label, seeds, bridges, rationale }] }`, hydrates each as a multi-seed `CodeUmbrella` + `agentic.bridge` edges, retires the absorbed singleton flows, and returns `{ flows: [{ id, members: [{ symbol_path, name }] }] }`. `--apply-descriptions <json>` consumes `{ descriptions: [{ symbol_path, text }] }` and persists them through the scoped write path.
- [x] #4 **`drift-sync` SKILL.md orchestrates the two judgements.** The skill drives the sub-agent through phase 1 (list → judge-stitch → apply-stitch) and phase 2 (judge-describe → apply-descriptions). The "no model call is required" framing is removed; the skill documents that the agent explores the codebase to judge stitches and authors descriptions.
- [x] #5 **The sub-agent can explore.** `drift-reconciler` gains `Read` and `Grep` (alongside `Skill`, `Bash`) and its instructions describe the two-phase flow. An empty inventory (no orphans, no unresolved sites) short-circuits both judgement phases and the result is byte-identical to today's deterministic output.
- [x] #6 **Stitch persistence is unchanged downstream.** A confirmed stitch reuses `build_bridge_edges` (`BRIDGE_CONFIDENCE_INFERRED = 0.5`, call-site span as `source_range`) and `hydrate_code_flow` → `write_flow` / `write_agentic_substrate`; membership re-induces across the bridge (`induce_members`); the umbrella id is its dominant seed's `symbol_path` (`flow_id_of`); identity stays purely deterministic.
- [x] #7 **Proving target (deterministic).** `reconcile_stitch.test.ts` is rewritten to the script boundary: feeding `--apply-stitch` a golden `umbrellas` JSON over the split-entrypoint fixture yields one multi-seed umbrella with the bridge over the missed call; feeding it no umbrellas (or running `--list-entrypoints` alone) leaves the orphans as singleton flows. No executor mock remains.
- [x] #8 **Docs updated.** `docs/comprehension/flow-construction.html` describes the skill-orchestrated two-phase model and the three bin modes; the agentic-stitching companions reflect the purge (no executor seam, no agenda layer).
<!-- AC:END -->
...
Ariadne is a syntactic call-graph extractor: registry lookups, dynamic dispatch, and callback wiring frequently leave call sites unresolved, so the callee is promoted to its own top-level entrypoint and one functionality fragments into several singleton flows. Repairing that fragmentation needs judgement — which fragments are one functionality, and what each member does — and judgement lives in a model context, not in a synchronous headless bin. The architecture follows that fact: the `drift-sync` skill orchestrates the drift-reconciler sub-agent (tools: Skill, Bash, Read, Grep) through two judgement phases over three dumb store-facing bin modes, and the engine's never-wired in-process executor seams plus the candidate-pairing/union-find substrate that fed them are deleted outright (net ≈ −1,250 lines).

The deterministic floor is hydrate-then-merge: `--list-entrypoints` runs the full deterministic reconcile (resync, retire, skill-dir, one singleton flow per new entrypoint — so an agent crash leaves a correct fragmented state and agent-less hosts keep getting flows), then emits the changed neighbourhood's entrypoint inventory with each tree's unresolved call sites (`is_unresolved_call`: no resolution lands on an in-graph node). The agent explores the codebase, writes `stitch.json`, and `--apply-stitch` hydrates each umbrella as a multi-seed `CodeUmbrella` (id = alphabetically-first resolved seed, never agent-chosen) with `agentic.bridge` edges at confidence 0.5, retiring the absorbed singletons; `reconcile()`'s `claimed_seed_paths` filter keeps them retired on every later pass. The evidence bar is enforced, not advisory: a bridge names its unresolved call site by inventory `file`/`line`, the bin corroborates the site against the live graph and derives the canonical provenance span itself, and an uncorroborated bridge never persists. `--apply-descriptions` upgrades the deterministic placeholders through the scoped write path; a byte-identical re-submission at an unchanged content hash is a cache no-op while a different text is a revision and writes.

Navigate from `packages/drift/src/reconcile/agentic_modes.ts` (the three modes' substance and wire types), then `bin/drift_reconcile.ts` (dispatch only), `reconcile.ts` (the deterministic engine), and `packages/drift/assets/skills/drift-sync/SKILL.md` (the orchestration the agent actually follows; `.claude/` copies are byte-identical mirrors). The proving target is `reconcile_stitch.test.ts` — ten cases at the script boundary, golden wire JSON over a real registry-dispatch fixture, no executor mocks. Deliberately not built: a hydration cap in `apply_stitch` (payloads are inventory-bounded), a cross-phase graph fingerprint (skew degrades to logged skips), and any agent say over flow identity.
