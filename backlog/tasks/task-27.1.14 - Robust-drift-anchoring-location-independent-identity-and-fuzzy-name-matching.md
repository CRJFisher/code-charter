...
The drift resolver that carries an agent-authored description across a code change is too brittle: it keys identity on the **`symbol_path` + `content_hash`** pair, and an Ariadne symbol's `symbol_path` is **location-bearing** — it encodes the enclosing scope and position. A function that is **moved unchanged** (same name, same body, new file or new position) therefore gets a new `symbol_path`, the exact match fails, and the resolver downgrades to `relocated` or, worse, `miss` — so the flow's identity churns and its description is needlessly re-generated (an LLM cost) instead of riding across the move. The `miss` rate in practice is higher than it should be because pure relocation defeats an exact-path scheme. (Post-strip, task-27.1.15, there is no re-attachment bin: a miss means the description is regenerated, not stranded — so the payoff here is identity stability and avoided regeneration cost, not recovering lost user content.)
...
2. **Edit-distance name tolerance (Levenshtein).** When the bare name changed (a rename), rank candidates by normalized edit distance on the name (plus structural / graph-proximity signals), so a small rename (`load_cfg` → `load_config`) re-anchors instead of missing. (task-27.1.15 removes the old `rank_candidates` helper — it only served the now-deleted re-attachment bin — so this task builds the candidate ranking it needs.)
...
- [ ] #2 When the bare name changed, candidates are ranked by normalized Levenshtein distance on the name, blended with structural / graph-proximity signals (this task builds the candidate ranking; the old `rank_candidates` helper is removed in task-27.1.15); a small rename re-anchors above the configured threshold.
...
3. Build the candidate-ranking module this task needs (the old `rank_candidates.ts` is removed in task-27.1.15) with normalized Levenshtein name distance as a ranking signal; keep it pure/dependency-light.
