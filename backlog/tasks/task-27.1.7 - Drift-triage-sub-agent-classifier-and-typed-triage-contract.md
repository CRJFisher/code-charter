...
- [ ] #1 The triage sub-agent is **spawned, not inline** (a Sonnet sub-agent via task-27.1.1's harness), so the main session is not blocked; topology is per-cluster workers + a merging orchestrator, keeping each agent's context small
...
3. Spawn per-cluster triage workers via task-27.1.1's `invoke_agent`; merge with an orchestrator; report verdicts back to task-27.1.5's drift rows (move to `triaged`).
