...
**What to watch.** The reconcile trigger is the `Stop` hook **alone** — no detached process, no auto-spawn (D-SUBAGENT-TRIGGER reverted). The hook's two re-fire guards are `stop_hook_active` and "nothing edited this turn"; scoping reconciliation to only-this-turn *across* turns (a watermark, so prior-turn edits aren't re-reconciled) lands in task-27.1.6, as does the `drift-sync` script body (here it no-ops/logs and honors its pinned contract). `drift.list`/`drift.resolve` ship a thin-but-real bin surface over existing store methods; the full bin semantics also land in 27.1.6. The installed hook commands carry absolute paths into the package's `dist/`, so v1 install is per-machine (portability across hosts/clones is task-27.1.8).
