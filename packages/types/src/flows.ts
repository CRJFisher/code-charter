/**
 * task-27.1.3 — the flow-keyed backend surface.
 *
 * A **flow** is v1's unit of comprehension: a functionality umbrella over one or more call-graph trees
...
/**
 * The rows a single flow renders to: its bounded call-graph subgraph folded by the file-module
 * scaffold. Exactly the shape the webview's `custom_graph_to_react_flow` adapter consumes.
 */
...
/** One entry in the left-panel flow selector (AC#7). */
export interface FlowSummary {
  /**
   * Stable flow id = the `symbol_path` of the dominant seed entrypoint (AC#4) — the location-free,
   ...
   */
  id: string;
  /** Display label: the dominant seed's name for a skeleton flow; the agent label once hydrated. */
  label: string;
  /** `EXISTS(agentic.flow node)` for this flow — drives hydrated-first ordering (AC#7). */
  is_hydrated: boolean;
  ...
  member_count: number;
  /** True for the single deterministic `unattributed` bucket (AC#8). */
