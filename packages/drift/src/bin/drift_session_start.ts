...
/**
 * Read the outstanding relocations from the store for `cwd`, degrading to empty on any failure,
 * including a fresh repo whose `.code-charter` store does not exist yet (opening it throws). The banner
 * is best-effort context, never a reason to disturb the session.
 */
function read_recoverable(cwd: string): { drift: DriftObservation[] } {
  let store;
  try {
  ...
  try {
    return { drift: outstanding_drift(store) };
  } catch {
    return { drift: [] };
  } finally {
    store.close();
...
async function main(): Promise<void> {
  ...
  const { drift } = read_recoverable(cwd);
  const output = build_session_start_output(drift);
